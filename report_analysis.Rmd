...
- Numero impianti connessi: 96.
- Dimensione matrice di adiacenza: 139x139.
- Numero impianti inclusi erroneamente: 1. _Questo impianto è incluso nel totale dei 139, ma non è stato incluso nelle analisi._

Con numero di impianti connessi si intende l'effettivo numero di nodi (impianti di risalita) che compongono la rete. Tramite la struttura dati del grafo (oggetto `tbl_graph`), eliminando ogni nodo con grado 0 (ovvero senza alcun collegamento con altri impianti di risalita), sono stati tenuti in considerazione solamente i dati dei rimanenti 96 nodi.
...
summary(res$centrality)

```

I quindici impianti più importanti sono: `r res[1:15,]$name`.
...
I dieci impianti più centrali secondo questo parametro sono: `r res[1:10,]$name`.

```{r Distribuzione centralita istogramma, echo=FALSE, fig.align='center', fig.retina=2, fig.width=12}
ggplot() +
  geom_col(data = res, aes(x = name, y = centrality, fill = 'blue'), alpha = 0.85) +
  geom_col(data = res[1:10,], aes(x = name, y = centrality, fill='red'), alpha = 1) +
  xlab("") + ylab('Centralita') +
  scale_fill_identity(name = '', guide = 'legend', labels = c('Tutti', 'Top')) +
...
for(i in 1:length(couples)){
  ...
  print(get_googlemap(center = c(n1$first_node_lon, n1$first_node_lat), zoom=13, key="AIzaSyBUQm6A_dAD5EVQZjvF1T43kHrusUVNKzI") %>%
    ggmap() +
      ...
      #geom_node_label(aes(x = n1$first_node_lon, y = n1$first_node_lat, label = impianti_analisi[couples[[i]][1],]$name), repel = TRUE, alpha = 0.2) +
      ...
      #geom_node_label(aes(x = n2$first_node_lon, y = n2$first_node_lat, label = impianti_analisi[couples[[i]][2],]$name), repel = TRUE, alpha = 0.2))
...
ggplot(res) +
  geom_col(aes(x = name, y = centrality, fill = 'black')) +
  geom_col(data = res1, aes(x = name, y = centrality, fill='red', color='red'), alpha = 0.2, show.legend = FALSE) +
  ylim(0, 3300) +
  xlab("") + ylab('Centralita') +
...
```{r Grafico di percolazione, echo=FALSE}
...
Mediamente all'interno di questa rete, la distanza tra due impianti qualsiasi è di `r mean_distance(g)`.
Questo vuol dire che uno sciatore in qualsiasi momento, per raggiungere un altro impianto qualsiasi (che non sia il direttamente successivo naturalmente) dovrà prendere nella media almeno `r mean_distance(g)` altri impianti.

Nel grafico sottostante è riportato l'istogramma delle distanze degli impianti della rete.
```{r Istogramma distanza, echo=FALSE}
